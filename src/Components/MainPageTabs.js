...
function CustomTabPanel(props) {
  ...
  return (
    ...
    >
      ...
      {value === index && (
        <Box sx={{ paddingTop: 3 }}> {/* Style pour le padding du contenu du panneau */}
          <Typography>{children}</Typography> {/* Afficher le contenu du panneau */}
        </Box>
      )}
    </div>
  );
...
export default function MainPageTabs({ title1, title2, children }) {
  ...
  return (
    <div> {/* Div conteneur pour les panneaux d'onglets */}
      <Box> {/* Boîte pour les onglets */}
        {/* Afficher les onglets avec les titres fournis */}
        <Tabs value={value} onChange={handleChange} aria-label="basic tabs example" centered>
          <Tab label={title1} {...a11yProps(0)} sx={classes.tab} /> {/* Premier onglet */}
          <Tab label={title2} {...a11yProps(1)} sx={classes.tab} /> {/* Deuxième onglet */}
        </Tabs>
      </Box>
      <Box sx={{ padding: 0 }}> {/* Boîte pour les panneaux d'onglets */}
        {/* Mapper les enfants (panneaux d'onglets) et les afficher */}
        {children.map((child) => (
          <CustomTabPanel value={value} index={i = i + 1} key={i}> {/* Panneau d'onglet personnalisé */}
